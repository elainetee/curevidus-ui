          ...
          <q-item v-for="post in posts" :key="post.id" class="q-py-md">
            ...
            <q-item-section>
              ...
              <q-item-label class="text-body1">
               ...
               <q-item-label caption  v-if="post.visibility == 'hidden'">Post is hidden</q-item-label>
  ...
  methods: {
    async hidePost(id) {
      try {
        ...
        this.$q.notify("Post hidden successfully");
        this.getPost();
        // this.posts = res.data;
      } catch (error) {
        console.log(error);
    ...
    async getPost() {
      try {
        const res = await this.$axios.get(
          `http://127.0.0.1:8000/api/allvisiblepost`,
          {
            headers: { Authorization: "Bearer" + Cookies.get("token") },
          }
        );
